(function () {
  ...
  const CONTACT_URL = "https://ai-assistant-backend-clean-iz6y.onrender.com/contact-human";
  ...
  box.innerHTML = `
    ...
    <button id="ai-human-button">📞 Parler à un humain</button>
  ...
  function getPageContent() {
    ...
    document.querySelectorAll("main, section, article, body").forEach(el => {
      if (el && el.innerText) text += el.innerText + "\n";
    });
  ...
  const humanButton = box.querySelector("#ai-human-button");

  humanButton.onclick = async () => {
    ...
    try {
      ...
      humanButton.innerText = "✅ Demande envoyée";
    } catch (err) {
      humanButton.disabled = false;
      humanButton.innerText = "📞 Parler à un humain";
